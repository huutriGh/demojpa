...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "demojpa-ear.ear");

        // Current directory is the root of the "demojpa-web" project. Go up one level
        // and enter the "demojpa-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../demojpa-ejb/target/demojpa-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "demojpa-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/demojpa-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "demojpa-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("org.aptech.test");
